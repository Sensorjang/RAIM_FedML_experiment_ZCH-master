...
def assign_edge_servers(initial_rewards, training_costs, reputations):
    ...
    for i in range(1, N): # 1 ~ N-1 原文2->N
        ...
        if best_cluster != -1:
            ...
            giveup_device.append(device_idx)

    # print("个体效用:" , ed_utilities)
    eds_total_utility = 0
    for i in range(N):
        if i in giveup_device:
            continue
...
def calculate_es_utilities(reputations, commun_costs, finalprice):
    """
    计算ES的效用
    """
...
def calculate_cs_utilities(finalprice):
    ...
    cs_utilities = gama * math.log(total_datasize + 1) - finalprice * total_datasize * 1e-6
...
def generate_reputations(N, lowrepu_ratio):
    ...
    return reputations
def raim(justsimulate, esnum = 0, ednum = 0, lowrepu_ratio = 0.0):
    ...
    calculate_es_utilities(reputations, commun_costs, finalprice)
